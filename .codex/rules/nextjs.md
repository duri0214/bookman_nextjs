...
- Jest / Testing Library のテストでは、後から読み返して意図が分かるように、各 `test` / `it` の直前または本文冒頭へ「シナリオ」をコメントで書く。
- シナリオには「入力」「処理」「期待値」を含める。単なる実装手順ではなく、ユーザー操作、API レスポンス、環境変数、props など、何を与えて何を確認するテストなのかを明確にする。
- テスト本文はシナリオに沿って Given / When / Then のまとまりが分かる構成にする。必要なら `// Given`, `// When`, `// Then` コメントを使う。
- テスト名は「正常に動く」だけで終わらせず、条件と期待結果が分かる日本語または英語にする。
- 実装詳細に寄りすぎたテストより、画面表示、API 変換、入力状態、エラー表示など、後から仕様として読めるテストを優先する。
