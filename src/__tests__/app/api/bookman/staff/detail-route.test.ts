...
describe('staff detail route', () => {
  ...
  test('PATCHが職員更新リクエストをbackend APIへ転送するべき', async () => {
    /**
     * シナリオ:
     * - 入力: 職員IDと更新後の支店、ロールを含む更新リクエスト。
     * - 処理: Next API route の PATCH を呼び出す。
     * - 期待値: backend の staff detail API へ同じ内容をPATCHすること。
     */
    ...
    const response = await PATCH(
      {
        json: async () => ({ name: '山田 太郎', branch: 2, role: 'admin' }),
      } as Request,
      { params: Promise.resolve({ staffId: '10' }) },
    )
